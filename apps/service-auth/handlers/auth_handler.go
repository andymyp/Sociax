...
func (h *Handlers) SignIn(body []byte) ([]byte, error) {
	_, span := h.tracer.Start(context.Background(), "SignIn")
	defer span.End()
	...
	res, rpcErr, err := h.service.SignIn(req)
	if err != nil {
		return rabbitmq.ErrorResponse(err.Error(), 500)
	}
	if rpcErr != nil {
...
func (h *Handlers) RefreshToken(body []byte) ([]byte, error) {
	_, span := h.tracer.Start(context.Background(), "RefreshToken")
	defer span.End()
	...
	res, rpcErr, err := h.service.RefreshToken(req)
	if err != nil {
		return rabbitmq.ErrorResponse(err.Error(), 500)
	}
	if rpcErr != nil {
...
func (h *Handlers) CheckAuth(body []byte) ([]byte, error) {
	_, span := h.tracer.Start(context.Background(), "CheckAuth")
	defer span.End()
	...
	rpcErr, err := h.service.CheckRefreshToken(req)
	if err != nil {
		return rabbitmq.ErrorResponse(err.Error(), 500)
	}
	if rpcErr != nil {
